...
z_stan <- stan_glm(dvote ~ gender, data = x,
                   refresh = 0, family = "binomial")
...
newdata <- tibble(gender = "female")

test1 <- predict(object = z_old, newdata = newdata,
                 type = "response")
...
q <- plogis(mean(posterior_linpred(z_stan, newdata = newdata)))


test3 <- posterior_linpred(z_stan, newdata = newdata,
                  transform = T)
